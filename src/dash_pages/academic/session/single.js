...
function Single() {
    ...
    useEffect(()=>{
        axios.get(`/api/web/dash/academic/session/${id}`)
            .then(v=>{
                 setdata(v.data.value)
                setloading(false)
            })
            .catch(e=>
   ...
   return(
     <>
            ...
            <MRouteView loading={loading} routes={[{name:"academic"},{name:"session",link:"/d/academic/session"}]} />  
